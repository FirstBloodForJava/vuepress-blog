...
$$
\text{当 p 是质数且 1 ≤ i ≤ p−1 时，有：}
\\
\binom{p}{i} ≡ \text{0 (mod p)}
\\
\text{} \binom{p}{i} = \frac{p!}{(p-i)!i!}
$$
**证明**：当 `1 ≤ i ≤ p−1` 时，$\frac{p!}{(p-i)!i!}$ 的分母是不包含 p 的。由于分子包含 p，且该结果为整数，所以 $\binom{p}{i}$ 可以被 p 整除。

**注**：如果 p 不是质数，p = 4，i = 2，$\binom{p}{i}$ = 6，不是 p = 4 的倍数。


$$
\text{对于任意整数 x 和 y 和任意质数 p，有 }\\
(x +y) ^ p = x^p + y^p \text{(mod p)} \\
\text{证明：}\\
(x + y)^p = \sum_{k=0}^{p} \binom{p}{k} x^{p-k}\\
=
$$
