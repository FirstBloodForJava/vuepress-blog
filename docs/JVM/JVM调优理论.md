...
启动参数：-Xmn1g -Xms4g -Xmx8g。一个程序会每10分钟在内存中创建100万个HashMap<Long,Long>对象(会占用 80MB 左右内存)，这个时候垃圾收集器的 Minor GC 会造成 500毫秒的停顿，出现这种收集时间长的问题，应该和新生代的对象复制到 Survivor 有关，应该是发生 GC 时这 800MB 对象没有被回收，而是被复制到老年代，才导致停顿时间这么长。

解决方法：

1. 让对象直接进入老年代，Major 程序去处理，避免新生代内存复制消耗，Major 停顿时间也不会短。
